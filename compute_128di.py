...
#   操作方法：把照片放在/home/zzy/face_rank/下 分别建立文件夹（可以是正常的照片不必为脸部），修改name=[]数组对应人的姓名，test.py中也需要修改
...
faces_folder_path = "/home/zzy/face_rank/face_score_pic/"

out = open("/home/zzy/face_rank/score_train.csv","w")
...
for k in range(0,score_level_num):
    for f in glob.glob(os.path.join(faces_folder_path+str(score[k]), "*.jpg")):
        img = io.imread(f)
        win.clear_overlay()
        win.set_image(img)
        dets = detector(img, 1)
