...
def loadCsv(filename):
    ...
    dataMat=[];labelMat=[];labelMat_1=[]  #label_1 是性别，倒数第二列，labelmat是评分最后一列
    dataset = list(lines)
...
if __name__ == '__main__':
    ...
    #     用图片测试
    frame=cv2.imread("/home/zzy/face_rank/face_score_test/2.png")
    ...
    try:
        ...
        score_index_1 = np.zeros(3)
        loca_dist2 = np.zeros(3)
        score_index_2 = np.zeros(3)
        print(new_dist1)
        ...
        labelMat[loca_dist1[0]]

        index=np.uint8(labelMat[[loca_dist1[j]]])
        print(index)
        print(score[index])
